...
作者：Jason Chia-Sheng Lin，Ph.D. Student，Institute of Biophotonics，National Yang Ming Chiao Tung University
...
## 6. Beyond Pass/Fail：評估未完成 Agent 狀態的品質

傳統 agent benchmark 很容易把任務壓成二元結果：成功或失敗、pass 或 fail。我認為這對 long-horizon agent system 不夠。長任務經常停在部分解決狀態：environment-recovery state、validated negative evidence、handoff artifact、被縮小的 hypothesis space，或尚未完成但可接手的 debugging trajectory。這些未完成狀態不能被視為等價。

我的核心主張是：

> 長時間 agent 不應只用是否抵達終點評估，也應評估它是否把 search space 留在更低 entropy、更可驗證、更可延續的狀態。

### 6.1 Unfinished state 與 failure quality

我把 **unfinished agent state** 定義為：一次未完成 run 結束後留下的 artifacts、verified facts、negative evidence、environment state、reasoning compression 與 continuation constraints 的集合。

我把 **failure quality** 定義為：一次不成功 run 對未來 run 減少不確定性的程度。高品質失敗會縮小 hypothesis space、排除 dead ends、保存 evidence、提升 reproducibility，並提高 continuation readiness。低品質失敗則會重複探索、增加 context entropy、遺失 evidence provenance，或留下模糊的 completion state。

| Run | Binary outcome | Unfinished-state quality |
| --- | --- | --- |
| Ubuntu Fugu Ultra | unfinished | 高探索，但低收斂、高 active-context entropy |
| Ubuntu GPT-5.5 handoff | unfinished | 高 state compression，明確區分 FACT / THEORY / REPORTED-UNVERIFIED |
| Mac ARM64 GPT-5.5 | unfinished | 高 negative-evidence preservation、environment reconstruction、reproducible harness 與 clean repo closeout |

三個 run 都沒有解出 exploit。如果只用 pass/fail benchmark，它們會全部被壓成 0 分。但實際工程價值完全不同。這正是本 packet 最值得提出的 evaluation signal。

### 6.2 能力失敗與 harness governance 失敗

我不把 Fugu trajectory 解讀為單純 capability failure。它同時是 harness governance failure。這次 run 缺少足夠硬的 hard budget、no-new-evidence stop rule、forced checkpoint、duplicate-command detector 與 proof-boundary gate，導致探索結果沒有及時轉成 durable state。

> This is not only a model problem. It is a harness governance problem.

更強的 governance layer 應要求 agent 定期外部化狀態、維護 failed-hypothesis ledger、在宣稱進展前檢查 official proof boundary，並要求延長探索必須產生新證據，而不是只產生更多 trace。

### 6.3 Entropy reduction 與 marginal evidence yield

在本報告中，entropy 指的是對 environment、exploit surface、failed hypotheses、valid proof boundaries 與 next-step action space 仍然存在的不確定性。好的 long-horizon agent 會隨著工作進展降低 entropy；漂移中的 agent 則可能持續消耗 token，卻只帶來很少新增證據。

```mermaid
flowchart LR
  A["探索量上升"] --> B["新證據增加"]
  B --> C["marginal evidence yield 趨緩"]
  C --> D["弱 governance：context entropy 上升"]
  C --> E["強 governance：state 被外部化"]
  D --> F["agent drift / 重複探索"]
  E --> G["reusable handoff / lower-entropy state"]
```

Fugu Ultra 確實降低了 ELF structure、coredump behavior、libc mappings、runtime mitigations 與 gadget paths 的不確定性；但這些降低被 repeated exploration、weak state compression 與 token amplification 部分抵消。GPT-5.5 則較有效地透過 verified facts compression、negative evidence preservation 與 repository artifacts 降低 entropy。

### 6.4 Handoff artifact quality rubric

為了讓這個觀點可重複評測，我建議採用 100 分 handoff artifact quality rubric：

| Dimension | Points | 評估問題 |
| --- | ---: | --- |
| Objective clarity | 10 | final success condition 是否明確、可測？ |
| Proof boundary | 15 | 是否區分 official proof、invalid proof 與 partial proof？ |
| Verified facts | 15 | confirmed facts 是否與 speculation、stale reports 分開？ |
| Failed paths | 15 | dead ends 是否有足夠細節避免重跑？ |
| Negative evidence | 15 | failed tests 是否 bounded、reproducible，並連到 observable outcomes？ |
| Reproducible commands | 10 | 下一個 agent 是否能重建環境或重跑檢查？ |
| Next-action contract | 10 | 是否縮小下一個 hypothesis 或 decision gate？ |
| Repository hygiene | 10 | files、commits、handoff paths 是否清楚？ |

這個 rubric 評估的是 run 結束後留下什麼，不只是 run 過程中做了什麼。它把 continuation readiness 變成 first-class metric。

### 6.5 超越 binary completion 的評估維度

| Metric | Description |
| --- | --- |
| Completion rate | final task 是否完成 |
| False-completion rate | agent 是否錯誤宣稱成功 |
| State compression quality | verified state 是否外部化為 durable artifacts |
| Negative-evidence preservation | failed paths 是否以可重用形式保存 |
| Continuation readiness | 下一個 agent 是否能避免 rediscovery 並直接接手 |
| Entropy reduction | 對 environment、hypothesis space 與 proof boundary 的不確定性移除程度 |
| Marginal evidence yield | 每增加 token、tool call 或時間區塊得到多少 useful evidence |
| Drift resistance | 避免 recursive rediscovery 與 generic status regression 的能力 |
| Proof-boundary discipline | official proof、sanity check、partial progress 與 speculation 是否分開 |
| Repo hygiene | artifact organization、commit、handoff path 是否清楚 |

這個框架重要，因為未來 agent 會進入 scientific workflows、software engineering、cybersecurity labs、autonomous debugging 與 infrastructure recovery。在這些場景中，一個誠實失敗且留下 reusable state 的 agent，可能比一個給出 unverifiable success claim 的 agent 更有價值。
...
### 7.5 後期出現 self-report calibration 問題
...
## 10. 這是不是 Codex CLI hierarchical memory 沒做好？
...
### 11.3 Fugu 可能沒有把 Codex 工具輸出視為必須主動壓縮的 working memory
...
### 13.4 Long-Horizon Native-Runtime Evaluation
...
### 13.6 Context Curation / Memory-as-Action
...
### 13.7 Tool-Result Clearing / Compaction / Memory Tool
...
### 13.8 Safety / Boundary Evaluation
...
### 13.9 Reliability / Drift / Loop Detection
...
### 13.11 Architecture Pattern Evaluation
...
### 13.12 Context Engineering / Cowork-style Analysis
...
### 17.3 是 Codex CLI hierarchical memory 沒做好，所以 Fugu memory 爆炸嗎？
...
### 17.4 為什麼 GPT-5.5 當 agent 比較沒有這問題？
