...
\section{Beyond Pass/Fail：評估未完成 Agent 狀態的品質}
...
\subsection{Unfinished state 與 failure quality}

我把 \finding{unfinished agent state} 定義為：一次未完成 run 結束後留下的 artifacts、verified facts、negative evidence、environment state、reasoning compression 與 continuation constraints 的集合。

我把 \finding{failure quality} 定義為：一次不成功 run 對未來 run 減少不確定性的程度。高品質失敗會縮小 hypothesis space、排除 dead ends、保存 evidence、提升 reproducibility，並提高 continuation readiness。低品質失敗則會重複探索、增加 context entropy、遺失 evidence provenance，或留下模糊的 completion state。

\begin{longtable}{L{0.30\linewidth} L{0.25\linewidth} L{0.35\linewidth}}
\toprule
...
\subsection{能力失敗與 harness governance 失敗}
...
\subsection{Entropy reduction 與 marginal evidence yield}
...
\end{axis}
\end{tikzpicture}
\caption{概念性 entropy curve：當 marginal evidence yield 下降後，弱 governance 會讓 context entropy 主導；強 governance 則會在 drift 擴大前把 state 外部化。}
\end{figure}
...
\subsection{超越 binary completion 的評估維度}
